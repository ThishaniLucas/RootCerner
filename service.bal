...
final fhirr4:FHIRConnector fhirConnectorObj = check new (cernerConfig);
...
service http:Service / on new http:Listener(9090) {
    ...
    isolated resource function post fhir/r4/[string resType](@http:Payload json|xml resPayload) returns http:Response {
        ...
        do {
            ...
            return handleError(e.message() + " : " + resType, http:STATUS_BAD_REQUEST);
        }
        if (rtype != resType) {
    ...
    isolated resource function patch fhir/r4/[string resType]/[string id](http:Request request, @http:Payload json|xml resPayload) returns http:Response {
        ...
        if (contentType is fhirr4:PatchContentType) {
            patchType = <fhirr4:PatchContentType>contentType;
        } else {
            return handleError("Unsupported Patch Content Type : " + contentType);
        }
